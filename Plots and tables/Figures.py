...
def plot_income_stacked_bars(act_data, impl_data, eco_data):
    ...
    plt.savefig("outputs/SI_Income_Groups_DimensionsL.png.pdf", dpi=300)
    plt.show()
...
# Sanity checks
required_base_cols = [
    "USD_Disbursement", "binary_label_biodiversity_impact", "No_Biodiv",
    "Year", "WB_Incomegroup", "FlowName"
]
...
# Ensure merged Act columns exist on ALL frames
def _ensure_merged_act_columns(_df):
    if _df is None:
        return
    for base_col in ["Act_Agri","Act_ForestMgmt","Act_Fish","Act_WaterMgmt","Act_OthMgmt",
                     "Act_Protect","Act_Resto","Act_Invasiv","Act_Pollut","Act_Undef"]:
        if base_col not in _df.columns:
            _df[base_col] = 0

    if 'Act_SustResMgmt' not in _df.columns:
        _df['Act_SustResMgmt'] = _df[['Act_Agri','Act_ForestMgmt','Act_Fish',
                                      'Act_WaterMgmt','Act_OthMgmt']].max(axis=1)
    if 'Act_Protect_Resto' not in _df.columns:
        _df['Act_Protect_Resto'] = _df[['Act_Protect','Act_Resto']].max(axis=1)

_ensure_merged_act_columns(df)


# Identify class columns
act_cols_all  = sorted([c for c in df.columns if c.startswith("Act_")])
impl_cols_all = sorted([c for c in df.columns if c.startswith("Impl_")])
eco_cols_all  = sorted([c for c in df.columns if c.startswith("Eco_")])
if not act_cols_all or not impl_cols_all or not eco_cols_all:
    raise ValueError("Missing Act_*, Impl_*, or Eco_* one-hot columns.")


act_cols_merged = ['Act_Protect_Resto', 'Act_SustResMgmt', 'Act_Invasiv', 'Act_Pollut', 'Act_Undef']
act_cols_use = [c for c in act_cols_merged if c in df.columns]
if not act_cols_use:
    act_cols_use = act_cols_all


# Base biodiversity filter
df_filt = df[
    (df["USD_Disbursement"] > 0) &
    (df["binary_label_biodiversity_impact"] == 1) &
    (df["No_Biodiv"] != 1) &
...
# Helper: proportional split within a dimension
def split_disbursement_by_dimension(df_in: pd.DataFrame, class_cols: list, class_label: str):
    """Split each row's disbursement equally across all *active* classes in a dimension."""
    d = df_in.copy()
    cols = [c for c in class_cols if c in d.columns]
    if not cols:
        return pd.DataFrame(columns=["WB_Incomegroup","FlowName","Year",class_label,"USD_Disb_Split"])

    d["active_dim_count"] = d[cols].sum(axis=1)
    d = d[d["active_dim_count"] > 0].copy()
    if d.empty:
        return pd.DataFrame(columns=["WB_Incomegroup","FlowName","Year",class_label,"USD_Disb_Split"])

    d["USD_split"] = d["USD_Disbursement"] / d["active_dim_count"]

    out_rows = []
    take_cols = ["WB_Incomegroup", "FlowName", "Year", "USD_split"]
    for _, r in d.iterrows():
        for c in cols:
            if r[c] == 1:
                out_rows.append({
                    "WB_Incomegroup": r["WB_Incomegroup"],
                    "FlowName": r["FlowName"],
                    "Year": r["Year"],
                    class_label: c,
                    "USD_Disb_Split": r["USD_split"],
                })
    return pd.DataFrame(out_rows, columns=["WB_Incomegroup","FlowName","Year",class_label,"USD_Disb_Split"])


overall = (
    df_filt.groupby(["WB_Incomegroup","FlowName","Year"])["USD_Disbursement"].sum()
          .groupby(["WB_Incomegroup","FlowName"]).mean().reset_index()
)

pivot_overall = (
    overall.pivot(index="WB_Incomegroup", columns="FlowName", values="USD_Disbursement")
...
long_act  = split_disbursement_by_dimension(df_filt, act_cols_use,  "ActClass")
long_impl = split_disbursement_by_dimension(df_filt, impl_cols_all, "ImplClass")
long_eco  = split_disbursement_by_dimension(df_filt, eco_cols_all,  "EcoClass")

def annual_avg_by_class(long_df: pd.DataFrame, class_key: str):
    if long_df.empty:
        return pd.DataFrame(columns=["WB_Incomegroup","FlowName",class_key,"USD_Annual_Avg_Bn","Share_within_IncomeFlow"])
    tmp = (
        long_df.groupby(["WB_Incomegroup","FlowName",class_key,"Year"])["USD_Disb_Split"].sum()
               .groupby(["WB_Incomegroup","FlowName",class_key]).mean().reset_index()
    )
    tmp.rename(columns={"USD_Disb_Split":"USD_Annual_Avg_Bn"}, inplace=True)
    # shares within income×flow
    tmp["Share_within_IncomeFlow"] = (
        tmp.groupby(["WB_Incomegroup","FlowName"])["USD_Annual_Avg_Bn"]
           .transform(lambda s: s / s.sum() if s.sum() > 0 else 0)
    )
    return tmp.sort_values(["WB_Incomegroup","FlowName","USD_Annual_Avg_Bn"], ascending=[True,True,False])

act_avg  = annual_avg_by_class(long_act,  "ActClass")
impl_avg = annual_avg_by_class(long_impl, "ImplClass")
eco_avg  = annual_avg_by_class(long_eco,  "EcoClass")


# ONE CHART (in middle panel of a 3-panel layout)
out_dir = "./outputs"
os.makedirs(out_dir, exist_ok=True)

# Income labels with line breaks
...
# Normalize to percentages per income group (stack to 100%)
pivot_percent = pivot_overall.div(pivot_overall.sum(axis=1), axis=0).fillna(0) * 100
# Absolute totals for each bar (USD bn)
abs_totals = pivot_overall.sum(axis=1)

# 3-panel layout, draw middle chart and place legend in right panel
fig, axs = plt.subplots(1, 3, figsize=(18, 6), sharex=False)  # ↓ reduced height from 12 → 8

# Left panel (off)
axs[0].axis("off")

# Right panel will hold legend
axs[2].axis("off")

ax = axs[1]  # middle panel for main chart

x = np.arange(len(income_groups))
bar_width = 0.65

palette = ["#d9d9d9", "#bfbfbf", "#999999", "#737373", "#1a1a1a"]
...
for i, flow in enumerate(flow_order):
    ...
    bars = ax.bar(
        ...
        color=palette[i % len(palette)], label=flow
    )

    # Add inside percentage labels
    for bar, hgt, y0 in zip(bars, heights, bottom):
        if hgt > 5:
            ax.text(
                ...
                f"{hgt:.0f}%",
                ha="center", va="center",
                fontsize=11, color="white" if i >= 2 else "black", fontweight="bold"
            )
...
# Totals above bars
for xi, total in zip(x, abs_totals.values):
    ax.text(
        xi, 106, f"${total:.1f}B",
        ha="center", va="bottom",
...
ax.set_ylim(0, 115)
ax.tick_params(axis="y", labelsize=12)
ax.set_ylabel(f"Annual average disbursement (USD bn and %, {start_year_of_interest}-{end_year_of_interest})", fontsize=12)
...
# Layout with even spacing
plt.tight_layout(rect=[0, 0, 1, 1])

# Save
plt.savefig(os.path.join(out_dir, "SI_Income_Group_Finance_Flow_Type.png"), dpi=300)
plt.savefig(os.path.join(out_dir, "SI_Income_Group_Finance_Flow_Type.pdf"), dpi=300)
plt.show()

# Excel output (summary + detailed sheets)
excel_path = os.path.join(out_dir, f"IncomeGroup_FlowType_ByDimensions_{start_year_of_interest}_{end_year_of_interest}.xlsx")
with pd.ExcelWriter(excel_path, engine="xlsxwriter") as writer:
    # Summary: Flow × Income (annual averages)
    summary = pivot_overall.copy()
    summary.index = [income_label_map.get(idx, idx) for idx in summary.index]
    summary.to_excel(writer, sheet_name="Summary_Flow_by_Income")

    # Details (map income labels for readability)
    out_act  = act_avg.copy();  out_act["WB_Incomegroup"]  = out_act["WB_Incomegroup"].map(income_label_map)
    out_impl = impl_avg.copy(); out_impl["WB_Incomegroup"] = out_impl["WB_Incomegroup"].map(income_label_map)
    out_eco  = eco_avg.copy();  out_eco["WB_Incomegroup"]  = out_eco["WB_Incomegroup"].map(income_label_map)

    out_act.to_excel(writer,  sheet_name="Act_by_Income_Flow",  index=False)
    out_impl.to_excel(writer, sheet_name="Impl_by_Income_Flow", index=False)
    out_eco.to_excel(writer,  sheet_name="Eco_by_Income_Flow",  index=False)
